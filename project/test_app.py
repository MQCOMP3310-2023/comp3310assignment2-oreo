...
def test_submit_invalid_rating(client):
    ...
    response = client.post(
        url_for('main.rate_restaurant', restaurant_id=restaurant.id), data={'value': 6})
...
def test_submit_valid_rating(client):
    ...
    response = client.post(
        url_for('main.rate_restaurant', restaurant_id=restaurant.id), data={'value': 4})
...
def test_resubmitting_rating_not_allowed(client):
    ...
    existing_ratings_count = Rating.query.filter_by(
        restaurant_id=restaurant.id).count()

    # Submit a valid rating for the restaurant
    response = client.post(url_for('main.rate_restaurant', restaurant_id=restaurant.id), data={
    ...
    new_ratings_count = Rating.query.filter_by(
        restaurant_id=restaurant.id).count()
